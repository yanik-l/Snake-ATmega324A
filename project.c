...
//Define the colour of the super-food
...
void splash_screen(void) {
	...
	printf_P(PSTR("CSSE2010/7201 Snake Project by Yanik Laube"));	
	...
	while(1) {
		set_scrolling_display_text("43920600", colour);
...
void new_game(void) {
	...
	// Initialise the score and print it
	init_score();
	print_score();
	
	//Initialise the snake length
	init_snake_length();
...
void play_game(void) {
	...
	//uint32_t super_food_initial_time;
	//uint32_t super_food_time;
	...
	while(1) {
		...
		//super_food_initial_time = get_clock_ticks();
		if(get_clock_ticks()%15000 == 0) {
			spawn_superfood();
		}
		// || (get_clock_ticks()-5000)%15000 == 0
		...
		if(button==0 || escape_sequence_char=='C') {
			...
			last_move_time = get_clock_ticks();
			uint32_t move_delay = get_game_speed() - (get_clock_ticks()-last_move_time);
			serial_input = -1;
			while(1) {
		...
		if(get_clock_ticks() >= last_move_time + get_game_speed()) {
